...
// Desktop prefers the bundled `liexiu` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/kailonyang/liexiu/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "liexiu.exe" : "liexiu";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `liexiu-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `liexiu` binary. If one is already present at
