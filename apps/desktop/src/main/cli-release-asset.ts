const RELEASE_ARCHIVE_PREFIX = "liexiu-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `liexiu-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `liexiu_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `liexiu_${os}_${mappedArch}.${ext}`;
