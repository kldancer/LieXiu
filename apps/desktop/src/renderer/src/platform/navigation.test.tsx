...
// what projects sessions into the single router. Auth stays mocked so we can
// spy on logout without coupling this test to the session transport.
...
import { useNavigation } from "@liexiu/views/navigation";
...
describe("openInNewTab", () => {
  ...
  it("delegates to switchWorkspace for a cross-workspace path", () => {
    ...
    getAdapter().openInNewTab!("/butter/projects");

    const s = useTabStore.getState();
    expect(s.activeWorkspaceSlug).toBe("butter");
    expect(getActiveTab(s)?.url).toBe("/butter/projects");
...
describe("push", () => {
  ...
  it("switches workspace for a cross-workspace path", () => {
    ...
    getAdapter().push("/butter/projects");
