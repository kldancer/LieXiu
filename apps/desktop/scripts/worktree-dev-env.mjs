...
// (keyed by the app name "LieXiu Canary"). The env hooks to override both
...
// "/tmp/liexiu-465" is 1035454000, and 1035454000 % 1000 === 0). Range 5174–6173.
...
// The dev app then shows e.g. "LieXiu Canary mul-3724-194" in Cmd+Tab and gets
...
// a basename at different paths (e.g. /a/liexiu vs /b/liexiu) or whose names
...
// the primary checkout keeps the unchanged 5173 / "LieXiu Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "LieXiu Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
