/**
 * MUL-5208 — the web half of the `liexiu:navigate` bridge.
 *
...
import { useNavigation, type NavigationAdapter } from "@liexiu/views/navigation";

function navigate(path: string) {
  window.dispatchEvent(
    new CustomEvent("liexiu:navigate", { detail: { path } }),
  );
}
...
describe("WebNavigationProvider internal link bridge", () => {
  ...
  it("ignores an event without a path", () => {
    ...
    window.dispatchEvent(new CustomEvent("liexiu:navigate", { detail: {} }));
 ...
 * or replaces with a fallback. A wrong `true` walks the user out of LieXiu,
