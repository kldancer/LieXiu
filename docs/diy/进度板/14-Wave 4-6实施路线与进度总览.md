...
**当前关键路径：Wave 4A.1。** Planner 先复用并泛化现有 Assignment/Run 执行链；Planner、Executor、Reviewer、
Integrator 的真实合同稳定后，才允许 Phaser 和高级游戏化占用主开发资源。

## 5. 总体进度

| Wave | 状态 | 完成项 | 退出结论 |
| --- | --- | ---: | --- |
| Wave 4：真实异构 Agent 团队 | ⬜ | 0 / 18 | Planner、Role/Runtime 路由和结构化协作闭环 |
| Wave 5：正式像素世界与回放 | ⬜ | 0 / 16 | Phaser 世界与 Command/Detail 使用同一 Projection |
| Wave 6：项目指挥中心与个人版 v1 | ⬜ | 0 / 13 | 项目级管理、真实多 Runtime 和长期运行验收 |
| **合计** | ⬜ | **0 / 47** | 当前从 4A.1 开始 |
...
| 4B.6 | 完成多厂商黄金场景 | ⬜ | 至少两种真实 Runtime 分担规划/执行/审查/集成，切换和离线恢复不改业务语义 |
...
| 编号 | 工作包 | 状态 | 退出条件 |
| --- | --- | --- | --- |
| 6B.1 | 固定两种以上真实 Runtime acceptance profile | ⬜ | 版本、capability、permission、模型和预算显式；默认测试仍不调用真实 CLI |
| 6B.2 | 验证异构角色团队完整交付 | ⬜ | 真实 Planner、Executor、Reviewer、Integrator 可跨 Runtime 完成黄金 Mission |
| 6B.3 | 验证失败与切换 | ⬜ | provider 错误、CLI 缺失、daemon 离线、timeout、权限拒绝、容量不足可恢复或 fail closed |
| 6B.4 | 决定 dsh bridge 最终边界 | ⬜ | 依据真实能力差距保留 Adapter-only 或发布薄 bridge，并有跨 Runtime 等价合同 |
