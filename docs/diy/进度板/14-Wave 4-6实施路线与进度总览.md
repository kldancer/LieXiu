...
**当前 goal 已完成：Wave 5–6 与个人版 v1 Gate 已收敛。** Wave 4A 与 4B 已跨 Gate。4B.6 的默认 PostgreSQL Golden 和显式
...
6C.1 已从固定 commit 完成远端无缓存双镜像构建、隔离 fresh install、350→351 升级、数据库/上传卷恢复和重启；
6C.2 已用真实优雅重启、有界 soak 与确定性断线/超时测试确认长期事实收敛。Wave 4–6 无未完成关键路径。
...
| Wave 6：项目指挥中心与个人版 v1 | ✅ | 13 / 13 | 项目级管理、真实多 Runtime、长期恢复、隔离升级与个人版 v1 发布 Gate 均已闭合 |
| **合计** | ✅ | **47 / 47** | Wave 4–6 与当前 goal 全部完成 |
...
Wave 6 Gate 已通过。长期运行采用“真实进程持续运行 + 优雅重启后的有界 soak + 时间前移的确定性超时/失效测试”组合，
而不是把真实过夜墙钟等待写成不可复现门禁；动态持续时间、远端主机、镜像 digest、恢复校验和清理结果只记录在
`../../../.work/tasks/wave5-6/receipt.md`。
