.PHONY: help makehelp dev server daemon cli liexiu build test realacceptance-multi-runtime migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree remove-worktree db-up db-down db-drop db-reset selfhost selfhost-build selfhost-stop
...
POSTGRES_DB ?= liexiu
POSTGRES_USER ?= liexiu
POSTGRES_PASSWORD ?= liexiu
POSTGRES_PORT ?= 5432
SQLC_VERSION ?= v1.31.1
PORT := $(or $(BACKEND_PORT),$(API_PORT),$(SERVER_PORT),$(PORT),8080)
ifeq ($(origin LIEXIU_PUBLIC_URL), undefined)
LIEXIU_PUBLIC_URL := http://localhost:$(PORT)
endif
FRONTEND_PORT ?= 3000
FRONTEND_ORIGIN ?= http://localhost:$(FRONTEND_PORT)
LIEXIU_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
LIEXIU_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@if [ ! -f .env ]; then \
		...
		OWNERKEY=$$(openssl rand -hex 32); \
		PGPASS=$$(openssl rand -hex 24); \
		VCSKEY=$$(openssl rand -base64 32); \
		if [ "$$(uname)" = "Darwin" ]; then \
			sed -i '' "s/^JWT_SECRET=.*/JWT_SECRET=$$JWT/" .env; \
			sed -i '' "s/^LIEXIU_OWNER_BOOTSTRAP_SECRET=.*/LIEXIU_OWNER_BOOTSTRAP_SECRET=$$OWNERKEY/" .env; \
			sed -i '' "s/^POSTGRES_PASSWORD=.*/POSTGRES_PASSWORD=$$PGPASS/" .env; \
			sed -i '' -E "s#^(DATABASE_URL=postgres://[^:]+:)[^@]*(@.*)#\1$$PGPASS\2#" .env; \
			sed -i '' "s#^LIEXIU_VCS_SECRET_KEY=.*#LIEXIU_VCS_SECRET_KEY=$$VCSKEY#" .env; \
		else \
			sed -i "s/^JWT_SECRET=.*/JWT_SECRET=$$JWT/" .env; \
			sed -i "s/^LIEXIU_OWNER_BOOTSTRAP_SECRET=.*/LIEXIU_OWNER_BOOTSTRAP_SECRET=$$OWNERKEY/" .env; \
			sed -i "s/^POSTGRES_PASSWORD=.*/POSTGRES_PASSWORD=$$PGPASS/" .env; \
		...
		echo "==> Generated random JWT_SECRET, LIEXIU_OWNER_BOOTSTRAP_SECRET, POSTGRES_PASSWORD, and LIEXIU_VCS_SECRET_KEY"; \
	fi
	@echo "==> Pulling official LieXiu images..."
	@if ! $(COMPOSE) -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${LIEXIU_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting LieXiu via Docker Compose..."
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building LieXiu from the current checkout..."
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) liexiu LIEXIU_ARGS="daemon restart --profile local"

cli: ## Run the liexiu CLI with ARGS or LIEXIU_ARGS from source
	@$(MAKE) liexiu LIEXIU_ARGS="$(LIEXIU_ARGS)"

liexiu: ## Run the liexiu CLI entrypoint directly from the Go source tree
	cd server && go run -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" ./cmd/liexiu $(LIEXIU_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/liexiu ./cmd/liexiu
...
realacceptance-multi-runtime: ## Run the explicitly authorized two-runtime Mission acceptance
	@bash scripts/run-multi-runtime-realacceptance.sh
...
sqlc: ## Regenerate sqlc code
	go -C server run github.com/sqlc-dev/sqlc/cmd/sqlc@$(SQLC_VERSION) generate
