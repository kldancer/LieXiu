// Brand host shown as the workspace URL prefix on the managed LieXiu Cloud,
...
// self-hosted server that has not set LIEXIU_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "liexiu.ai";
 ...
 * self-hosted instances show their own domain instead of `liexiu.ai`. Falls
