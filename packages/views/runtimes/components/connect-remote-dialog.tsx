...
import { runtimeKeys } from "@liexiu/core/runtimes/queries";
...
import { CODE_LIGATURE_CLASS } from "@liexiu/ui/lib/code-style";
...
const CLOUD_SERVER_URL = "https://api.liexiu.ai";
const CLOUD_APP_URL = "https://liexiu.ai";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `liexiu setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `liexiu config set server_url ${normalizedServerUrl}
liexiu config set app_url ${normalizedAppUrl}
liexiu login --token <YOUR_TOKEN>
liexiu daemon start`,
    };
  }

  return {
    setupCmd: "liexiu setup",
    tokenCmd: `liexiu config set server_url ${CLOUD_SERVER_URL}
liexiu config set app_url ${CLOUD_APP_URL}
liexiu login --token <YOUR_TOKEN>
liexiu daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `liexiu setup` is one blocking command that handles config + login
