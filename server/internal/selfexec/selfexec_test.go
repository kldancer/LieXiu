...
func TestResolveWithFallsBackToArgv0(t *testing.T) {
	...
	t.Run("absolute path", func(t *testing.T) {
		want := writeTestExecutable(t, t.TempDir(), "liexiu")
	...
	t.Run("relative path", func(t *testing.T) {
		dir := t.TempDir()
		want := writeTestExecutable(t, dir, "liexiu")
		t.Chdir(dir)
	...
	t.Run("PATH command", func(t *testing.T) {
		...
		got, err := resolveWith(failExecutable, []string{"liexiu"})
		if err != nil {
			t.Fatalf("resolveWith() error = %v", err)
		}
		if got != want {
...
func TestResolveWithRejectsInvalidFallback(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing executable", args: []string{"liexiu-does-not-exist"}, want: "liexiu-does-not-exist"},
	}
	...
	if runtime.GOOS != "windows" {
		t.Run("non-executable file", func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "liexiu")
