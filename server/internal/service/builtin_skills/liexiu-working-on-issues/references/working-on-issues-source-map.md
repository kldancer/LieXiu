...
## `liexiu issue pull-requests` — read PR links from LieXiu

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/liexiu/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/liexiu/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/liexiu/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `liexiu issue pull-requests` later reads back.
...
| Behavior | Source |
|---|---|
| `issue assign --no-start`, `issue update --no-start`, and `issue status --no-start` send `suppress_run=true` | `server/cmd/liexiu/cmd_issue.go` (`runIssueAssign`, `runIssueUpdate`, `runIssueStatus`) |
...
| `--stage` on `issue create` / `issue update` | `server/cmd/liexiu/cmd_issue.go:328,350` |
| `liexiu issue children <id>` (sub-issues grouped by stage) | `server/cmd/liexiu/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
assignee decides the parent is complete, the system
comment explicitly asks for `liexiu issue status <parent-id> in_review` —
...
| Behavior | File:line |
|---|---|
| `liexiu issue metadata set <issue-id> --key --value [--type]` | `server/cmd/liexiu/cmd_issue_metadata.go:80,109-111` |
| `liexiu issue metadata delete <issue-id> --key` | `server/cmd/liexiu/cmd_issue_metadata.go:93,113` |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/liexiu/cmd_issue.go
