---
name: liexiu-working-on-issues
description: "Use when acting on a LieXiu issue beyond what the brief covers: PR linking vs close intent, reading a linked PR's real state, metadata keys, status-change side effects, sub-issue todo vs backlog."
user-invocable: false
allowed-tools: Bash(liexiu *), Bash(git *), Bash(gh *)
---

# Working on LieXiu issues
...
For building mention links, load `liexiu-mentioning` instead — not this skill.
...
This is the link that `liexiu issue pull-requests` reads back — but see the
...
**excluded from `liexiu issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final LieXiu issue comment, unless
...
When a step depends on PR state, query LieXiu's link table — do not infer it
...
```bash
liexiu issue metadata set <issue-id> --key <key> --value <value>
liexiu issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `liexiu property list` shows the catalog;
  `liexiu issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
liexiu issue property set <issue-id> --name Environment --value staging
liexiu issue property set <issue-id> --name Platforms --value "iOS,Android"
liexiu issue property unset <issue-id> --name Environment
```
...
- **`in_progress` / `in_review` on assignment runs** are agent-managed CLI
  ...
  runtime brief asks agents for `todo`/`backlog` → `in_progress` then
  `in_review` when they have delivered.
...
```bash
liexiu issue assign <issue-id> --to-id <agent-id> --no-start
liexiu issue update <issue-id> --assignee-id <agent-id> --no-start
liexiu issue status <issue-id> in_progress --no-start
```
...
```bash
liexiu issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
liexiu issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
liexiu issue status <child-id> todo   # promote when the previous step is truly done
```
...
liexiu issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
liexiu issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
liexiu issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
liexiu issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
liexiu issue children <parent-id>             # sub-issues grouped by stage
liexiu issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
liexiu issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
liexiu issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
liexiu issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
liexiu issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
liexiu issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
