---
name: liexiu-creating-agents
description: "Use when creating, inspecting, or debugging a LieXiu agent definition via the `liexiu agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(liexiu *)
---

# Creating LieXiu agents

This is the contract for LieXiu's agent-creation path: what the create entry
...
```bash
liexiu agent get <agent-id> --output json      # full persisted agent record
liexiu agent skills list <agent-id> --output json   # current skill bindings
liexiu agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`liexiu agent create`). At task claim time the daemon
...
`liexiu agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
liexiu agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
liexiu agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
| `visibility` | `agent.visibility` | — | access control; defaults to `private`; gates who can read/route a private agent — NOT the runtime prompt |
...
```bash
liexiu agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
liexiu agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`liexiu agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`liexiu agent env set`), which carries the same
...
```bash
liexiu agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
liexiu agent update <agent-id> --mcp-config-stdin --output json
liexiu agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `liexiu agent create` — inserts a new agent row.
- `liexiu agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `liexiu agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `liexiu agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
