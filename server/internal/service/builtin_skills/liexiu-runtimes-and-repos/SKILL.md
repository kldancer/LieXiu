---
name: liexiu-runtimes-and-repos
description: "Use when a LieXiu runtime or daemon misbehaves: agent not running, task not claimed, runtime offline, workdir or session reuse, repository checkout."
user-invocable: false
allowed-tools: Bash(liexiu *)
---

# LieXiu Runtimes and Repos
...
7. `liexiu repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
liexiu runtime list --output json
liexiu runtime usage <runtime-id> --output json
liexiu runtime activity <runtime-id> --output json
liexiu runtime update <runtime-id> --target-version <version> --output json
liexiu runtime delete <runtime-id>
liexiu repo checkout <url>
liexiu repo checkout <url> --ref <branch-or-sha>
```

`runtime update` and `runtime delete` are writes. Starting a runtime update is limited to its owner or a workspace owner/admin; the original initiator may keep polling that specific in-flight request if their admin role changes. `runtime delete` removes a runtime registration; if active agents are still bound, it refuses unless the user explicitly passes `--cascade`, which unbinds those agents and cancels their queued/running tasks before deleting the runtime. Unbinding keeps the agents and everything they own — instructions, skills, labels, channel installations and task history — and only clears `agent.runtime_id`; an unbound agent cannot run until it is bound to a runtime again (`liexiu agent update <id> --runtime-id <runtime-id>`), and every trigger path refuses it with `agent_runtime_required`. `repo checkout` creates a dedicated branch in the task working directory. Most runtimes use a linked worktree; Linux and Windows Codex use task-local Git metadata so a task can stage and commit without making the shared `.repos` cache writable.

`repo checkout` requires `LIEXIU_DAEMON_PORT`; it is intended to run inside a daemon task. If absent, you are not in the normal agent checkout path. When a project `github_repo` resource has `resource_ref.ref`, `repo checkout <url>` uses that ref by default for the current task; an explicit `repo checkout <url> --ref <branch-or-sha>` overrides it.

## Task CLI boundary

The daemon injects a task-scoped `mat_` credential for LieXiu API commands and a private task-local LieXiu configuration root. Inside that managed task context:

- API commands such as `issue list`, `issue get`, and `issue runs` use the injected task identity and never fall back to the daemon Owner's saved LieXiu profile.
- `config show` and `config set` operate only on task-local LieXiu state. A missing task config root fails closed.
...
`LIEXIU_DAEMON_PORT` alone is a weak, defense-in-depth signal for task-safe API and profile resolution, not task identity for human/local command rejection. Genuine tasks also carry `LIEXIU_AGENT_ID` / `LIEXIU_TASK_ID`, `LIEXIU_TASK_CONFIG_ROOT`, or a daemon-managed workdir marker. When the port is the only signal, `daemon status` uses the selected host profile's derived health port; ordinary API and profile-resolving commands still fail closed. Host operators should not export `LIEXIU_DAEMON_PORT`; the daemon derives its host health port from the selected profile and injects the variable into tasks itself.
...
1. Was a task supposed to be created? Inspect issue/comment context.
2. Is the assignee an agent, and is it archived or bound to a runtime the actor cannot use?
3. Is the agent archived or bound to a runtime the actor cannot use?
4. Is the runtime online? `liexiu runtime list --output json`.
