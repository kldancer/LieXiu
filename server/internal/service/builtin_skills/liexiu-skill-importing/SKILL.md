---
name: liexiu-skill-importing
description: "Use when asked to import or install a specific skill into this LieXiu workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(liexiu *)
---

# Importing skills into LieXiu

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current LieXiu workspace.
...
A skill is installed for LieXiu only when it exists in the current workspace's
...
```bash
liexiu skill import --url <url> --output json              # hosted source
liexiu skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the LieXiu workspace DB, so LieXiu cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `liexiu skill import --url <url> --output json`:

```bash
liexiu skill import --url clawhub.ai/owner/skill --output json
liexiu skill import --url skills.sh/owner/repo/skill --output json
liexiu skill import --url github.com/owner/repo --output json
liexiu skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
liexiu skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`liexiu skill import --file <path> --output json` imports a skill from a local
...
After the final `liexiu agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`liexiu agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`liexiu agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `liexiu skill import --url <url>` is equivalent to
...
liexiu skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
liexiu skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
liexiu skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
liexiu skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
liexiu skill get <skill-id> --output json
```
...
## Updating an already-imported skill: `liexiu skill refresh`
...
```bash
liexiu skill refresh <skill-id> --output json
```
...
`liexiu skill get`), not the import result envelope. Failure modes to report
...
Incorrect (bypasses LieXiu):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but LieXiu cannot manage it as a workspace skill.
...
```bash
liexiu skill import --url https://skills.sh/owner/repo/skill --output json
```
