---
name: liexiu-mentioning
description: "Use when an issue comment needs to @mention someone — link to a person, trigger another agent, or broadcast with @all. Whether to mention at all is covered by the runtime brief, not here."
...
This skill states WHAT a mention link does in the LieXiu backend, traced to
...
exactly three `<type>` values plus the `all` sentinel, and the `<id>` group
accepts only hex characters and dashes, OR the literal string `all`:

    (member|agent|issue|all)/([0-9a-fA-F-]+|all)
...
at a project (see the liexiu-projects-and-resources skill); everything else in
this document is about the three types (plus `all`) the parser does recognize.

## Step 1 — look up the UUID with `--output json`

A name is not a UUID. Look the UUID up first, from the matching list command:

- a person → `liexiu workspace member list --output json` → use `user_id`
- an agent → `liexiu agent list --output json` → use `id`
...
## Step 2 — the three types and exactly what each enqueues
...
It acts on `agent` mentions only; `member` and `issue` mentions render links and
enqueue no task.
...
contract above: only `agent` mentions enqueue work.
...
`@all` only suppresses those IMPLICIT routes. An EXPLICIT `@agent`
...
- **A hex-ish but wrong UUID.** A well-formed-looking UUID that no entity owns
  ...
  **So when you see `invocation_not_allowed`, check the UUID against the live
  ...
  An id that matches the pattern but is NOT a valid
...
- **An already-pending task.** Even a correct `@agent` starts no second
  ...
  bypass.
- **An archived agent, or one with no runtime bound:** blocked with
  `target_unavailable` and `runtime_offline` respectively. Both are checked only
  AFTER the invoke gate, so a caller who may not invoke the target never learns
  its state.
- **A private agent you cannot invoke:** blocked — the mention path gates on
  `canInvokeAgent` for `@agent`. That is the *run* gate, not
  ...
  nothing about being able to mention it.
...
Correct:
  1. `liexiu workspace member list --output json`  → Alice's `user_id` = 7f3a…
