...
func TestHelperStatusErrorsAreClassified(t *testing.T) {
	...
	statusCases := []struct {
		...
		{http.StatusUnauthorized, ExitAuth, "liexiu login"},
...
// context that truncated a longer LIEXIU_HTTP_TIMEOUT.
func TestCommandContextNotTruncatedBelowHTTPTimeout(t *testing.T) {
	...
	t.Setenv("LIEXIU_HTTP_TIMEOUT", "400ms")
...
// LIEXIU_HTTP_TIMEOUT and the AtLeast floor.
func TestAPITimeoutRespectsEnv(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		t.Setenv("LIEXIU_HTTP_TIMEOUT", "")
	...
	t.Run("AtLeast budget wins when larger than floor", func(t *testing.T) {
		t.Setenv("LIEXIU_HTTP_TIMEOUT", "120s")
	...
	// responsive, but it must still honor LIEXIU_HTTP_TIMEOUT (it is not a
