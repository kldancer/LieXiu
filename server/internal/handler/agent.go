...
type AgentResponse struct {
	...
	// Instructions is what this agent's owner wrote.
...
type AgentTaskResponse struct {
	...
	// regardless of task source — sees the same
	...
	TriggerSummary        *string                `json:"trigger_summary,omitempty"`         // canonical short description snapshot taken at task creation; survives source edits/deletes
	...
	// comment's author. Empty for task kinds with no attributable human
	// initiator (on-assign, quick-create). InitiatorEmail is set only for member initiators
	...
	// the owner. The agent's effective LieXiu credentials stay owner-scoped —
	...
	Kind           string `json:"kind"`                      // discriminator: "comment" | "direct"
	...
	// LIEXIU_TOKEN in the agent process environment. The server binds it to
...
type TaskAttribution struct {
	...
	// direct_human | delegation | comment_source | owner_fallback |
	...
	// Originator is the authorization human (originator_user_id).
...
// TaskEvidence is the kind-tagged handle to a run's direct cause.
...
// or issue) — we ask the caller to resolve it once and pass it
...
// to choose how to render an issue task.
...
type CreateAgentRequest struct {
	...
	// `agent_created` analytics event. Empty
...
type UpdateAgentRequest struct {
	...
	// (MUL-3963). Owner-only writes: a non-owner admin passing them is silently
	// ignored, because the invoke
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["custom_env"]; ok {
		writeError(w, http.StatusBadRequest, "custom_env is no longer accepted on this endpoint; use PUT /api/agents/{id}/env (or `liexiu agent env set`)")
		return
	}
	...
	// who can run their agent, so admin-authored access would be confusing and
	// unsafe).
	//
...
// optional type query selects issue work; omitting it keeps the all-sources
// projection. scope=mine narrows issue work to the authenticated
...
func (h *Handler) ListWorkspaceWorkingAgents(w http.ResponseWriter, r *http.Request) {
	...
	case "", "issue":
	default:
		writeError(w, http.StatusBadRequest, "invalid type: must be issue")
		return
	}
...
// only feeds the agent activity card's "last activity" line. Cancelled tasks are
