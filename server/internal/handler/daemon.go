...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // liexiu CLI version
...
func (h *Handler) repairStaleCommentPlanIfNeeded(ctx context.Context, task *db.AgentTaskQueue, runtimeWorkspaceID string) (handled bool, failure *claimBuildFailure) {
	...
	_, cancelErr := h.TaskService.CancelTask(ctx, task.ID)
	if cancelErr != nil {
		return true, &claimBuildFailure{outcome: "error_stale_comment_plan", status: http.StatusInternalServerError, message: "failed to repair stale comment task"}
	}
	return true, nil
...
func (h *Handler) ClaimTasksByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	for i := range claimed {
		...
		if failure != nil {
			// Builder rejected this task (workspace isolation / retired historical kind);
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	// owner). The daemon will inject this as LIEXIU_TOKEN into the agent
...
func (h *Handler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	...
	// The terminal transaction is committed.
...
func (h *Handler) emitIssueExecutedOnFirstCompletion(r *http.Request, task *db.AgentTaskQueue) {
	...
	// prefix so local event consumers do not confuse them with a user.
...
// task cancellation and finished flushing the transcript. It records any
// terminal branch/worktree delivery and re-broadcasts the cancelled task so
// clients see those late fields; the acknowledgement is otherwise idempotent.
...
// Quick-create tasks have no parent record at WriteGCMeta time, so the daemon
// keys GC directly on the task row itself.
