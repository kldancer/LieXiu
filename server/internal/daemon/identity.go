...
// same machine shares one UUID stored at `~/.liexiu/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.liexiu/profiles/*/daemon.id` and returns every
...
// explicitly pins LIEXIU_DAEMON_ID to the hostname itself, there's nothing
