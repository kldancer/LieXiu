...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "liexiu login") {
		t.Fatalf("401 warning must tell the user to run 'liexiu login', got: %s", out)
	}
}
...
// and the operator would never see a "run liexiu login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "liexiu login") {
		t.Fatalf("expected the actionable 'run liexiu login' hint in the WARN, got: %s", out)
	}
}
