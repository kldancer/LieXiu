...
const (
	...
	// hard ceiling for cost/resource control can set LIEXIU_AGENT_TIMEOUT.
	...
	// Set LIEXIU_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set LIEXIU_AGENT_TOOL_WATCHDOG=0 to
...
// LIEXIU_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // liexiu CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/liexiu_workspaces)
	...
	GCCodexSessionTTL              time.Duration         // reclaim a per-issue Codex session store (~/.codex/liexiu-sessions/<agent>/<issue>) untouched for at least this long, so a done/abandoned issue's conversation history does not accumulate forever (default: 14d, set 0 to disable)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on LieXiu Cloud, false on self-host)
	AutoUpdateCheckInterval        time.Duration         // how often the auto-update loop polls for a new release (default: 6h)
	AutoReloadEnabled              bool                  // restart when the liexiu binary on disk no longer matches the running version (default: true for CLI-launched daemons)
	PollInterval                   time.Duration
	HeartbeatInterval              time.Duration
	AgentTimeout                   time.Duration
	CodexSemanticInactivityTimeout time.Duration
	// CodexFirstTurnNoProgressTimeout is an explicit override for the Codex
	// first-turn no-progress ceiling (LIEXIU_CODEX_FIRST_TURN_TIMEOUT). 0 means
	...
	// written by `liexiu runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("LIEXIU_SERVER_URL", DefaultServerURL)
	...
	//
	//   - LIEXIU_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	// users already get with LIEXIU_OPENCLAW_PATH today. We achieve it with
	...
	claudeArgs, err := shellArgsFromEnv("LIEXIU_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("LIEXIU_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("LIEXIU_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenArgs, err := shellArgsFromEnv("LIEXIU_QWEN_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenpawArgs, err := shellArgsFromEnv("LIEXIU_QWENPAW_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("LIEXIU_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("LIEXIU_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("LIEXIU_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("LIEXIU_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexFirstTurnNoProgressTimeout, err := durationFromEnv("LIEXIU_CODEX_FIRST_TURN_TIMEOUT", 0)
	if err != nil {
		return Config{}, err
	}
	...
	if codexFirstTurnNoProgressTimeout > 0 {
		...
		if codexFirstTurnNoProgressTimeout >= effectiveSemanticTimeout {
			slog.Warn("LIEXIU_CODEX_FIRST_TURN_TIMEOUT is greater than or equal to the semantic-inactivity timeout; the effective first-turn wait is truncated to the semantic timeout and the model-catalog startup retry is disabled. Because the semantic timer is armed first and equal durations do not deterministically favour the first-turn deadline, set LIEXIU_CODEX_SEMANTIC_INACTIVITY_TIMEOUT strictly above LIEXIU_CODEX_FIRST_TURN_TIMEOUT (with some margin) to preserve it.",
	...
	codexHandshakeTimeout, err := durationFromEnv("LIEXIU_CODEX_HANDSHAKE_TIMEOUT", DefaultCodexHandshakeTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// LIEXIU_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("LIEXIU_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}
	// LIEXIU_OPENCODE_IDLE_WATCHDOG narrows the no-message window for
	// OpenCode's streamed model responses. Zero removes the provider-specific
	// override and falls back to LIEXIU_AGENT_IDLE_WATCHDOG; positive values
	// cannot extend the global bound, and the global zero still disables the
	// whole mechanism.
	openCodeIdleWatchdog, err := durationFromEnv("LIEXIU_OPENCODE_IDLE_WATCHDOG", DefaultOpenCodeIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// LIEXIU_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("LIEXIU_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("LIEXIU_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via LIEXIU_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("LIEXIU_DAEMON_ID"))
	...
	// — e.g. ~/.liexiu/daemon.id (default) plus ~/.liexiu/profiles/<x>/
	...
	// the user explicitly pins LIEXIU_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("LIEXIU_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("LIEXIU_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/liexiu_workspaces or ~/liexiu_workspaces_<profile>)
	...
	keepEnv := os.Getenv("LIEXIU_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("LIEXIU_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("LIEXIU_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("LIEXIU_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("LIEXIU_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("LIEXIU_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("LIEXIU_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcCodexSessionTTL, err := durationFromEnv("LIEXIU_GC_CODEX_SESSION_TTL", DefaultGCCodexSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcHermesMemoryTTL, err := durationFromEnv("LIEXIU_GC_HERMES_MEMORY_TTL", DefaultGCHermesMemoryTTL)
	if err != nil {
		return Config{}, err
	}
	gcHermesSessionTTL, err := durationFromEnv("LIEXIU_GC_HERMES_SESSION_TTL", DefaultGCHermesSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcRepoTTL, err := durationFromEnv("LIEXIU_GC_REPO_TTL", DefaultGCRepoTTL)
	if err != nil {
		return Config{}, err
	}
	gcRepoMaintenanceEnabled := boolFromEnv("LIEXIU_GC_REPO_MAINTENANCE_ENABLED", true)
	gcArtifactPatterns := patternsFromEnv("LIEXIU_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on LieXiu Cloud (api.liexiu.ai) and opt-out for
	...
	// Operators on either side can flip the default with LIEXIU_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := boolFromEnv("LIEXIU_DAEMON_AUTO_UPDATE", isOfficialCloudServer(serverBaseURL))
	if overrides.DisableAutoUpdate {
		autoUpdateEnabled = false
	}
	autoUpdateInterval, err := durationFromEnv("LIEXIU_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
	...
	autoReloadEnabled := boolFromEnv("LIEXIU_DAEMON_AUTO_RELOAD", true)
...
// officialCloudHost is the hostname of LieXiu's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.liexiu.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.liexiu.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at LieXiu's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid LIEXIU_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("LIEXIU_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
const TaskWorkspacesRootEnv = "LIEXIU_TASK_WORKSPACES_ROOT"
...
// LIEXIU_WORKSPACES_ROOT env > default ($HOME/liexiu_workspaces, or
// $HOME/liexiu_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `liexiu daemon disk-usage`) use this directly so they pick the same
...
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("LIEXIU_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set LIEXIU_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "liexiu_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "liexiu_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("LIEXIU_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// When ~/.liexiu/hooks shadows a real agent binary, skip that hooks directory:
...
func resolveAgentExecutablePath(cmd string) (string, error) {
	...
	if isInLieXiuHooksDir(resolved) {
		if unshadowed, err := lookPathExcludingLieXiuHooks(cmd); err == nil {
...
// probe() order in LoadConfig: exec.LookPath (with the ~/.liexiu/hooks
...
func reresolveAgentCommand(cmd string) (string, bool) {
	...
	// this — an operator-pinned LIEXIU_*_PATH that no longer exists should
...
func lookPathExcludingLieXiuHooks(cmd string) (string, error) {
	for _, dir := range filepath.SplitList(os.Getenv("PATH")) {
		if dir == "" {
			dir = "."
		}
		if isLieXiuHooksDir(dir) {
			continue
		}
...
func isInLieXiuHooksDir(path string) bool {
	if path == "" {
		return false
	}
	return isLieXiuHooksDir(filepath.Dir(path))
}

func isLieXiuHooksDir(dir string) bool {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return false
	}
	return samePathDir(dir, filepath.Join(home, ".liexiu", "hooks"))
}
...
// before any LIEXIU_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom LIEXIU_*_PATH values never reach this
...
//     name is kept,
//  6. if the resolved path lives in ~/.liexiu/hooks, searches the same
...
func buildLoginShellResolveScript(names []string) string {
	...
	b.WriteString("  hc=\"\"\n")
	b.WriteString("  if [ -n \"${HOME:-}\" ]; then hd=\"$HOME/.liexiu/hooks\"; hc=$(cd \"$hd\" 2>/dev/null && pwd -P) || hc=\"\"; fi\n")
...
//
//   - The two vars touched (LIEXIU_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("LIEXIU_OPENCLAW_PATH"); !set {
			_ = os.Setenv("LIEXIU_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
