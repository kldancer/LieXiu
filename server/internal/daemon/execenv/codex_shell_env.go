...
// daemon-spawned Codex process can have LIEXIU_TOKEN while `liexiu issue ...`
...
const (
	...
	liexiuShellEnvBeginMarker = "# BEGIN liexiu-managed shell-environment (do not edit; regenerated by daemon)"
	liexiuShellEnvEndMarker   = "# END liexiu-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited LIEXIU_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// LIEXIU_* values are safe to include because daemon.go blocklists that
...
func renderLieXiuShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(liexiuShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(liexiuShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderLieXiuShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
