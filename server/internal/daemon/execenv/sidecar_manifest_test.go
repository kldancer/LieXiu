...
// nothing related to LieXiu before a task ran must contain nothing
// related to LieXiu after the task finishes — no .agent_context/,
// no .claude/skills/, no .liexiu/, no stub directories. The test
...
// must still exist and the LieXiu-written sibling must be gone.
...
// round-trip to the .liexiu/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .liexiu/sidecar.txt and created
	// .liexiu/ + .liexiu/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .liexiu/.
	managedDir := filepath.Join(workDir, ".liexiu")
	...
	// .liexiu still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-liexiu/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug LieXiu would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a LieXiu skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the LieXiu
// sibling (which lives at `<slug>-liexiu`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "LieXiu's version",
						Content:     "---\nname: issue-review\n---\n\nLieXiu skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a LieXiu-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.liexiu/project/
// resources.json` — the other LieXiu-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".liexiu", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".liexiu", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-liexiu`, then `-liexiu-2`,
// `-liexiu-3`, … The PR-review concern is "LieXiu skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-liexiu`.
	...
	if slug != "issue-review-liexiu" {
		t.Errorf("second allocation should bump to `-liexiu`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-liexiu") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-liexiu"), 0o755); err != nil {
	...
	if slug != "issue-review-liexiu-2" {
		t.Errorf("third allocation should be `-liexiu-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-liexiu-2") {
...
// `Issue Review` skill, the LieXiu sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// LieXiu sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the LieXiu skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "LieXiu's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	liexiuDir := filepath.Join(workDir, ".claude", "skills", "issue-review-liexiu")
	if _, err := os.Stat(filepath.Join(liexiuDir, "SKILL.md")); err != nil {
		t.Errorf("LieXiu sibling skill should exist at %s: %v", liexiuDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// LieXiu sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(liexiuDir); !os.IsNotExist(err) {
		t.Errorf("LieXiu sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
