...
//
// Two kinds, mutually exclusive in practice. classifyTask documents the
// tiebreak rule that applies if a future caller accidentally violates the
// mutex.
type taskKind int

const (
	// kindIssue: this run operates on a real LieXiu issue. It deliberately
...
// hasIssueContext returns true for the kinds that operate on a real LieXiu
...
//   - Sub-issue Creation
//
// The quick-create mode has no current issue and would either render an
// empty body or steer the
