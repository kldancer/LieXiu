...
// — users cannot audit or edit the contents from any LieXiu UI.
//
// This conflicts with the LieXiu daemon's context model. LieXiu
...
//     unrelated host-project context into LieXiu tasks. The reproduction
//     in github.com/kailonyang/liexiu#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new LieXiu issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a LieXiu task
// (and accept the leak risk) can keep the feature enabled by setting
// `LIEXIU_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a LieXiu-owned, user-visible,
...
// LieXiuCodexMemoryEnv is the env var users can set to keep Codex
...
const LieXiuCodexMemoryEnv = "LIEXIU_CODEX_MEMORY"

const (
	liexiuMemoryFeatureBeginMarker = "# BEGIN liexiu-managed memory-feature (do not edit; regenerated by daemon)"
	liexiuMemoryFeatureEndMarker   = "# END liexiu-managed memory-feature"
	liexiuMemoryConfigBeginMarker  = "# BEGIN liexiu-managed memory-config (do not edit; regenerated by daemon)"
	liexiuMemoryConfigEndMarker    = "# END liexiu-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(LieXiuCodexMemoryEnv))
...
// renderLieXiuMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderLieXiuMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(liexiuMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(liexiuMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderLieXiuMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderLieXiuMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(liexiuMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(liexiuMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderLieXiuMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderLieXiuMemoryConfigBlock(true))
}
...
//
// When LIEXIU_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per LIEXIU_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderLieXiuMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderLieXiuMemoryConfigBlock(false))
	}
