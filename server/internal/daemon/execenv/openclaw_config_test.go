...
// MUL-2219 DoD calls out: the directory LieXiu writes skills into MUST be
...
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — LieXiu's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
...
//
// When a liexiu agent is configured for gateway-mode openclaw and the
