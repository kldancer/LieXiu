...
// subagents. The LieXiu daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the LieXiu runtime
...
//
// Users who explicitly want Codex native subagents inside a LieXiu task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `LIEXIU_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// LieXiuCodexMultiAgentEnv is the env var users can set to keep Codex
...
const LieXiuCodexMultiAgentEnv = "LIEXIU_CODEX_MULTI_AGENT"

// liexiuMultiAgentBeginMarker / liexiuMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	liexiuMultiAgentBeginMarker = "# BEGIN liexiu-managed multi-agent (do not edit; regenerated by daemon)"
	liexiuMultiAgentEndMarker   = "# END liexiu-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(LieXiuCodexMultiAgentEnv))
...
// renderLieXiuMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderLieXiuMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(liexiuMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(liexiuMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderLieXiuMultiAgentBlock(true)
...
//
// When LIEXIU_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per LIEXIU_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderLieXiuMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
