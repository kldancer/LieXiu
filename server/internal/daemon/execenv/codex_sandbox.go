...
// `no such host` errors when calling out (for example, `liexiu issue get`
// hitting the LieXiu API). See upstream issue openai/codex#10390.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `liexiu issue
...
//     danger-full-access so the LieXiu CLI can reach the API.
...
// liexiuManagedBeginMarker / liexiuManagedEndMarker delimit the block the
...
const (
	liexiuManagedBeginMarker = "# BEGIN liexiu-managed (do not edit; regenerated by daemon)"
	liexiuManagedEndMarker   = "# END liexiu-managed"
)

// renderLieXiuManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.liexiu]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertLieXiuManagedBlock), avoids both traps.
func renderLieXiuManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(liexiuManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(liexiuManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuManagedEndMarker) + `\n*`)

// upsertLieXiuManagedBlock returns the config content with the liexiu-managed
...
// `[permissions.liexiu]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderLieXiuManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertLieXiuManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderLieXiuManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the liexiu-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertLieXiuManagedBlock(existing, policy)
