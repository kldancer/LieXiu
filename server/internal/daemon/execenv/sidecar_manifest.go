...
const sidecarManifestFile = ".liexiu_sidecar_manifest.json"
...
//     so the agent still discovers the LieXiu skill, just under a
//     different directory name.
//   - For LieXiu-only namespaces (.agent_context/issue_context.md,
//     .liexiu/project/resources.json) the caller swallows the error
...
// down a LieXiu skill without colliding with a user-installed skill
...
// collision we append `-liexiu`, then `-liexiu-2`, `-liexiu-3`,
...
// baseSlug: the bare slug first, then `-liexiu`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-liexiu"
	default:
		return fmt.Sprintf("%s-liexiu-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .liexiu/, .claude/skills/, .github/skills/,
...
// repository, so "left on disk" means a marker that disables every liexiu
...
// issue-review-liexiu. A managed skill directory is platform-owned — the
