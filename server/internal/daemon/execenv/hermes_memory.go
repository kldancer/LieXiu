...
// agent writes back) in `<HERMES_HOME>/memories/`. LieXiu has to take over
...
//
// This file makes memory an agent-scoped store LieXiu owns:
//
//	<liexiu profile dir>/hermes-state/<agent>/<hermes profile>/memories
//
...
// hermesMemoryStoreRoot is the directory under the daemon's LieXiu profile dir
...
// agent to key on, or the LieXiu profile dir cannot be resolved. The daemon
// marks the returned path in-use for the task's duration so
// PruneHermesMemoryStores never reclaims it mid-mount.
//
// daemonProfile namespaces by LieXiu profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the LieXiu profile dir can be on
...
// LieXiu profile dir), so it must actively replace a link left by a previous
