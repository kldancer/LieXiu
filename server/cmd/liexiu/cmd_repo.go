...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("LIEXIU_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("LIEXIU_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("LIEXIU_WORKSPACE_ID")
	agentName := os.Getenv("LIEXIU_AGENT_NAME")
	taskID := os.Getenv("LIEXIU_TASK_ID")
	...
	reqBody := map[string]any{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("LIEXIU_REPO_CHECKOUT_MODE")),
		"retry_busy":    true,
	}
	...
	for {
		...
		if resp.StatusCode == http.StatusServiceUnavailable && resp.Header.Get("X-LieXiu-Retryable") == "repo-busy" {
