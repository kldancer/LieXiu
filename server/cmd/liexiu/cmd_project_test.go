...
// review round 2: `liexiu project resource update <p> <r> --default-branch-hint x`
...
func TestBuildResourceRefFromFlagsGithubMergesHint(t *testing.T) {
	t.Run("hint-only edit preserves existing url", func(t *testing.T) {
		...
		existing := map[string]any{"url": "https://github.com/kailonyang/liexiu"}
	...
	t.Run("url override survives merge", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/kailonyang/new-repo")
		existing := map[string]any{
			"url":                 "https://github.com/kailonyang/liexiu",
		...
		if ref["url"] != "https://github.com/kailonyang/new-repo" {
...
func TestBuildResourceRefFromRefFlagKeepsJSONEscapeHatch(t *testing.T) {
	t.Run("json payload wins over github shortcuts", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/kailonyang/ignored")
		_ = cmd.Flags().Set("ref", `{"url":"https://github.com/kailonyang/liexiu","ref":"release/v2"}`)
		...
		if payload["url"] != "https://github.com/kailonyang/liexiu" {
	...
	t.Run("broken json is still rejected", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/kailonyang/liexiu")
