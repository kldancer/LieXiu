...
func TestHumanLocalCommandDistinguishesPortHintFromTaskIdentity(t *testing.T) {
	...
	t.Setenv(cli.TaskConfigRootEnv, filepath.Join(t.TempDir(), "task-liexiu"))
...
func TestHumanLocalCommandRejectsExplicitTaskIdentity(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "agent ID blocks setup", envName: "LIEXIU_AGENT_ID", command: "setup"},
		{name: "task ID blocks daemon stop", envName: "LIEXIU_TASK_ID", command: "daemon stop"},
	} {
...
func TestMissingServerConfigMessageExplainsPortOnlyContext(t *testing.T) {
	...
	if !strings.Contains(message, "LIEXIU_DAEMON_PORT") || !strings.Contains(message, "remove") {
...
// impersonation escape: a sandbox fault strips every LIEXIU_* env var from
// an agent subprocess, which then runs `liexiu` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.liexiu marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("LIEXIU_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (LIEXIU_AGENT_ID / LIEXIU_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.liexiu/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("LIEXIU_AGENT_ID", "agent-123")
		t.Setenv("LIEXIU_TASK_ID", "task-456")
		t.Setenv("LIEXIU_DAEMON_PORT", "")
		t.Setenv("LIEXIU_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("LIEXIU_AGENT_ID", "")
		t.Setenv("LIEXIU_TASK_ID", "")
		t.Setenv("LIEXIU_DAEMON_PORT", "27182")
		t.Setenv("LIEXIU_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without LIEXIU_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without LIEXIU_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("LIEXIU_TASK_ID", "task-456")
		t.Setenv("LIEXIU_DAEMON_PORT", "")
		t.Setenv("LIEXIU_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want LIEXIU_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("LIEXIU_TASK_ID", "")
		t.Setenv("LIEXIU_TOKEN", "")
		t.Setenv("LIEXIU_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want LIEXIU_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// LIEXIU_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// LIEXIU_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("LIEXIU_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("LIEXIU_AGENT_ID", "")
		t.Setenv("LIEXIU_TASK_ID", "")
		t.Setenv("LIEXIU_TOKEN", "")
		t.Setenv("LIEXIU_DAEMON_PORT", "")
		t.Setenv("LIEXIU_SERVER_URL", "https://api.liexiu.ai")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("LIEXIU_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("LIEXIU_WORKSPACE_ID", "workspace-123")
	t.Setenv("LIEXIU_AGENT_ID", "agent-123")
	t.Setenv("LIEXIU_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("LIEXIU_TOKEN", "mul_member_token")
...
func TestNewAPIClient_DaemonPortRequiresTaskToken(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "LIEXIU_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("LIEXIU_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("LIEXIU_WORKSPACE_ID", "test-ws")
	t.Setenv("LIEXIU_TOKEN", "test-token")
	t.Setenv("LIEXIU_AGENT_ID", "")
	t.Setenv("LIEXIU_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "liexiu agent env set") {
		t.Fatalf("no-fields error must direct users to `liexiu agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `liexiu agent env set` instead", flag)
		}
	}
}
...
func TestAgentMaxConcurrentTasksFlagValidation(t *testing.T) {
	...
	t.Setenv("LIEXIU_WORKSPACE_ID", "ws-1")
	t.Setenv("LIEXIU_TOKEN", "test-token")
	t.Setenv("LIEXIU_AGENT_ID", "")
	t.Setenv("LIEXIU_TASK_ID", "")
