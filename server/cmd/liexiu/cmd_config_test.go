...
func TestRunConfigSetPersistsSupportedKeysInProfile(t *testing.T) {
	...
	workspacesRoot := filepath.Join(t.TempDir(), "liexiu-dev")
...
func TestApplyConfigSetSupportsDaemonKeys(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	workspacesRoot := filepath.Join(t.TempDir(), "liexiu")
...
func TestApplyConfigSetNormalizesWorkspacesRoot(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	if err := applyConfigSet(&cfg, "workspaces_root", filepath.Join("data", "liexiu")); err != nil {
		t.Fatalf("applyConfigSet: %v", err)
	}
	want := filepath.Join(cwd, "data", "liexiu")
...
func TestApplyConfigSetPositiveDurationRoundTripsToDaemonResolver(t *testing.T) {
	const envName = "TEST_LIEXIU_PERSISTED_DURATION"
	t.Setenv(envName, "")
