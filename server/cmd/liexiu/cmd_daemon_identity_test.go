...
const blockingChildEnv = "LIEXIU_TEST_BLOCKING_CHILD"
...
// carries no platform build tag, so `go test ./cmd/liexiu` has to pass there
...
func TestDaemonStatusInTaskContextReportsNamedProfileHost(t *testing.T) {
	setup := func(t *testing.T) {
		...
		t.Setenv("LIEXIU_DAEMON_PORT", strconv.Itoa(stub.port))
	}
