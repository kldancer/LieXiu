...
func TestRunRepoCheckoutForwardsManagedCheckoutMode(t *testing.T) {
	...
	t.Setenv("LIEXIU_DAEMON_PORT", strings.TrimPrefix(srv.URL, "http://127.0.0.1:"))
	t.Setenv("LIEXIU_WORKSPACE_ID", "ws-1")
	t.Setenv("LIEXIU_AGENT_NAME", "Test Agent")
	t.Setenv("LIEXIU_TASK_ID", "task-1")
	t.Setenv("LIEXIU_REPO_CHECKOUT_MODE", "isolated")
...
func TestRunRepoCheckoutRetriesServiceUnavailable(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attempts++
		if attempts == 1 {
			w.Header().Set("X-LieXiu-Retryable", "repo-busy")
