...
func withAgentContext(t *testing.T) {
	t.Helper()
	t.Setenv("LIEXIU_TASK_ID", "task-1")
}
...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			body: "the PR is at [#42](https://github.com/kailonyang/liexiu/pull/42)",
		},
		{
			name: "mention link",
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No LIEXIU_AGENT_ID / LIEXIU_TASK_ID: a person running the CLI. Their
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`liexiu issue update` cannot carry files — deliver the file with `liexiu issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`liexiu issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "liexiu issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "liexiu issue update --attachment") {
