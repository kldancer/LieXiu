...
// ~/.liexiu/profiles/<name>/daemon.log while ~/.liexiu/daemon.log may still
...
func TestDaemonLogSourcePathResolvesPerProfile(t *testing.T) {
	t.Run("default profile", func(t *testing.T) {
		...
		want := filepath.Join(home, ".liexiu", "daemon.log")
		if got != want {
	...
	t.Run("named profile", func(t *testing.T) {
		home := mkProfiles(t, "desktop-api.liexiu.ai")

		got, err := daemonLogSourcePath("desktop-api.liexiu.ai")
		if err != nil {
			t.Fatalf("daemonLogSourcePath = %v, want nil", err)
		}
		want := filepath.Join(home, ".liexiu", "profiles", "desktop-api.liexiu.ai", "daemon.log")
		if got != want {
			t.Fatalf("daemonLogSourcePath = %q, want %q", got, want)
		}
	})
...
func TestRunDaemonLogsAnnouncesResolvedPath(t *testing.T) {
	cases := []struct {
		...
		{"named profile", "desktop-api.liexiu.ai", "desktop-api.liexiu.ai", 50, false},
		{"named profile following", "desktop-api.liexiu.ai", "desktop-api.liexiu.ai", 50, true},
	}
...
// The announcement goes to stderr, so `liexiu daemon logs | grep ...` and a
...
func TestRunDaemonLogsMissingFileNamesProfilePath(t *testing.T) {
	...
	seedDaemonLog(t, filepath.Join(home, ".liexiu", "daemon.log"))

	cmd, errOut := daemonLogsCmdFor(t, "desktop-api.liexiu.ai", 50, false)
	rec := stubTailLog(t, errOut)
