...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `liexiu issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `liexiu issue comment add` by its command
...
// `liexiu issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "liexiu issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "liexiu" && !strings.HasSuffix(executable, "/liexiu") {
		return false
	}
...
// like `LIEXIU_TOKEN=x liexiu issue comment add ...` is still recognized.
