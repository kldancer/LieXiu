...
const (
	liexiuCodexMcpBeginMarker = "# BEGIN liexiu-managed mcp_servers (do not edit; regenerated by daemon)"
	liexiuCodexMcpEndMarker   = "# END liexiu-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(liexiuCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(liexiuCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = liexiuCodexMcpBeginMarker + "\n" + liexiuCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(liexiuCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(liexiuCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isLieXiuMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "liexiu-agent-sdk",
				"title":   "LieXiu Agent SDK",
				"version": "0.2.0",
			},
...
//
// An explicit configured override (LIEXIU_CODEX_FIRST_TURN_TIMEOUT) is honored
