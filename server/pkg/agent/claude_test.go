...
func TestMergeEnvFiltersClaudeCodeVars(t *testing.T) {
	...
	env := mergeEnv([]string{
		...
		"LIEXIU_SERVER_URL=https://daemon.example",
	}, map[string]string{
		"FOO":                "bar",
	...
	if !found["LIEXIU_SERVER_URL=https://task.example"] || !found["LIEXIU_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task LIEXIU_* values to be appended, got %v", env)
	}
}
...
func TestResumeWasRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verbatim from kailonyang/liexiu#5704 (Claude Code 2.1.207,
...
func TestBuildClaudeArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildClaudeArgs(ExecOptions{
		ClaudeSettingsPath: "/tmp/liexiu-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/liexiu-claude-settings.json") {
