...
// the liexiu CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `liexiu issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest liexiu CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("liexiu CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("liexiu CLI version is below required minimum")
)
