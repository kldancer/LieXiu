# LieXiu installer for Windows — one command to get started.
#
# Install CLI (default): connects to liexiu.ai
#   irm https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.ps1 | iex
#
# Self-host: starts a local LieXiu server + installs CLI + configures
#   $env:LIEXIU_MODE="local"; irm https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/kailonyang/liexiu.git"
$RepoWebUrl    = "https://github.com/kailonyang/liexiu"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".liexiu\server"
$InstallDir    = if ($env:LIEXIU_INSTALL_DIR) { $env:LIEXIU_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/kailonyang/liexiu/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:LIEXIU_SELFHOST_REF) {
        return $env:LIEXIU_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = liexiu version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing LieXiu CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "LieXiu requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/kailonyang/liexiu/releases/download/$latest/liexiu-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "liexiu-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "liexiu.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/kailonyang/liexiu/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "liexiu.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "liexiu-cli-$version-windows-$arch.zip"
        $legacyAsset = "liexiu_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "liexiu.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".liexiu\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "liexiu.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "liexiu.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "liexiu.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "liexiu.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "LieXiu CLI installed to $binDir\liexiu.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "liexiu") {
        ...
        if ($isUpToDate) {
            Write-Ok "LieXiu CLI is up to date ($currentVer)"
            return
        }

        Write-Info "LieXiu CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "LieXiu CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "liexiu")) {
        Write-Fail "CLI installed but 'liexiu' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. LieXiu self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:LIEXIU_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up LieXiu server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning LieXiu repository..."
    ...
    Write-Info "Pulling official LieXiu images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting LieXiu services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "LieXiu server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  LieXiu - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] LieXiu CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     liexiu setup               " -NoNewline; Write-Host "# Connect to LieXiu Cloud (liexiu.ai)" -ForegroundColor DarkGray
    Write-Host "     liexiu setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:LIEXIU_MODE="with-server"; irm https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  LieXiu - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] LieXiu server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     liexiu setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Bootstrap: set LIEXIU_OWNER_BOOTSTRAP_SECRET in .env (at least 32 bytes),"
    Write-Host "  then open the frontend to bind the local owner and canonical workspace."
    Write-Host ""
    Write-Host "  To stop all services:"
    Write-Host '     $env:LIEXIU_MODE="stop"; irm https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping LieXiu services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No LieXiu installation found at $InstallDir"
    }

    if (Test-CommandExists "liexiu") {
        try {
            liexiu daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:LIEXIU_MODE) { $env:LIEXIU_MODE.ToLower() } else { "default" }
