...
_setup_sandbox() {
  ...
  cat >"$payload_dir/liexiu" <<'STUB'
#!/usr/bin/env bash
echo "liexiu v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/liexiu"
  tar -czf "$tmp/liexiu.tar.gz" -C "$payload_dir" liexiu

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/kailonyang/liexiu/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    LIEXIU_BIN_DIR="$tmp/install-bin" \
    LIEXIU_TEST_ARCHIVE="$tmp/liexiu.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/liexiu" ]]; then
    echo "expected fallback binary at $tmp/install-bin/liexiu" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
...
STUB
  ...
  if ! grep -q "https://liexiu.ai/settings?tab=tokens" "$tmp/install.out"; then
  ...
  if ! grep -q "liexiu login --token <YOUR_TOKEN>" "$tmp/install.out"; then
    echo "expected token login command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "liexiu config set server_url" "$tmp/install.out"; then
    echo "did not expect default cloud server config command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "liexiu config set app_url" "$tmp/install.out"; then
...
_run_with_server() {
  ...
  if ! env -i \
    ...
    HOME="$tmp" \
    LIEXIU_INSTALL_DIR="$tmp/server" \
    LIEXIU_SELFHOST_REF="main" \
    LIEXIU_TEST_CURL_LOG="$tmp/curl.log" \
    "$@" \
