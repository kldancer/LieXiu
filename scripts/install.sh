#!/usr/bin/env bash
# LieXiu installer — installs the CLI and optionally provisions a self-host server.
#
# Install / upgrade CLI only:
#   curl -fsSL https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.sh | bash
#
# Install CLI + provision self-host server:
#   curl -fsSL https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.sh | bash -s -- --with-server
#
# After installation, run `liexiu setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/kailonyang/liexiu.git"
REPO_WEB_URL="https://github.com/kailonyang/liexiu"  # without .git, for GitHub web APIs
INSTALL_DIR="${LIEXIU_INSTALL_DIR:-$HOME/.liexiu/server}"
BREW_PACKAGE="kailonyang/tap/liexiu"
...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://liexiu.ai/settings?tab=tokens${RESET}\n"
  printf "        and create a token under ${BOLD}Settings > API Tokens${RESET}.\n"
  printf "     2. On this server, run:\n"
  printf "        ${CYAN}liexiu login --token <YOUR_TOKEN>${RESET}\n"
  printf "        ${CYAN}liexiu daemon start${RESET}\n"
  printf "\n"
}
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). LieXiu supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing LieXiu CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap kailonyang/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "LieXiu CLI already installed via Homebrew"
    else
      warn "Failed to install liexiu via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "LieXiu CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing LieXiu CLI from GitHub Releases..."
  ...
  local url="https://github.com/kailonyang/liexiu/releases/download/${latest}/liexiu-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/liexiu.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/liexiu.tar.gz" -C "$tmp_dir" liexiu

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with LIEXIU_BIN_DIR.
  local bin_dir="${LIEXIU_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/liexiu" "$bin_dir/liexiu"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/liexiu" "$bin_dir/liexiu"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/liexiu" "$bin_dir/liexiu"
    chmod +x "$bin_dir/liexiu"
  ...
  rm -rf "$tmp_dir"
  ok "LieXiu CLI installed to $bin_dir/liexiu"
}
...
get_selfhost_ref() {
  if [ -n "${LIEXIU_SELFHOST_REF:-}" ]; then
    printf '%s' "$LIEXIU_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading LieXiu CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "LieXiu CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "LieXiu CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists liexiu; then
    local current_ver
    # `liexiu version` outputs "liexiu 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(liexiu version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "LieXiu CLI is up to date ($current_ver)"
      return 0
    fi

    info "LieXiu CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(liexiu version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "LieXiu CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists liexiu; then
    fail "CLI installed but 'liexiu' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. LieXiu self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning LieXiu repository..."
  ...
  info "Pulling official LieXiu images..."
  pull_official_selfhost_images
  info "Starting LieXiu services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  LieXiu — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ LieXiu CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}liexiu setup${RESET}                # Connect to LieXiu Cloud (liexiu.ai)\n"
  printf "     ${CYAN}liexiu setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  print_remote_server_token_hint
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  LieXiu — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ LieXiu server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}liexiu setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  printf "  ${BOLD}Bootstrap:${RESET} set ${CYAN}LIEXIU_OWNER_BOOTSTRAP_SECRET${RESET} in .env (at least 32 bytes),\n"
  printf "  then open the frontend to bind the local owner and canonical workspace.\n"
  printf "\n"
  printf "  ${BOLD}To stop all services:${RESET}\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/kailonyang/liexiu/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No LieXiu installation found at $INSTALL_DIR"
  fi

  if command_exists liexiu; then
    liexiu daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the LieXiu CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  LIEXIU_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.liexiu/server)"
        echo "  LIEXIU_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  LIEXIU_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'liexiu setup' to configure your environment."
        exit 0
        ;;
