...
$runnerScript = Join-Path ([System.IO.Path]::GetTempPath()) "liexiu-install-ps1-case.ps1"
...
if (-not $env:USERPROFILE) { $env:USERPROFILE = [System.IO.Path]::GetTempPath() }
$env:LIEXIU_INSTALL_DIR = $WorkDir
$env:LIEXIU_SELFHOST_REF = "main"
...
foreach ($case in $cases) {
    $workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("liexiu-ps1-" + [guid]::NewGuid().ToString("N"))
