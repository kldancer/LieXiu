...
test("onboarding — welcome → about you (answer path)", async ({ page }) => {
  ...
  await page.addInitScript((t) => {
    localStorage.setItem("liexiu_token", t);
  }, token);
  ...
  await expect(page.getByText("What do you want to use LieXiu for?")).toBeVisible();
  ...
  await expect(page.getByText("How did you hear about LieXiu?")).toHaveCount(0);
...
test("onboarding — one skip clears the whole questionnaire step", async ({ page }) => {
  ...
  await page.addInitScript((t) => localStorage.setItem("liexiu_token", t), token);
  await page.goto("/onboarding", { waitUntil: "domcontentloaded" });
  await waitForPageText(page, "Continue on web");
